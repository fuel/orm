...
class Model implements \ArrayAccess, \Iterator {
	...
	/**
	 * Generates a ClassName_ID string for recreating the model from only the key
	 *
	 * @return string
	 */
	public function get_model_key()
	{
		if (!$this->is_new())
		{
			if (array_key_exists('id', static::properties()))
			{
	...
	/**
	 * Created an instance of a model from its get generated by ->get_model_key()
	 */
	public static function model_from_key($key)
	{
		$parts = explode('_', $key);
		
		// We should always have at least two parts
		if (count($parts) < 2)
		{
			return false;
		}
